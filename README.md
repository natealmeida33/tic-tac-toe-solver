...
4. Run `npm start` or `npm run start` at the root.
...
2. Run `npm build` or `npm run build` at the root to transpile all TypeScript into JavaScript.
3. Run `npm test` or `npm run test` at the root.
